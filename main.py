...
class User(NoSQLModel):
    ...
    emailverified:bool = Field(False, description="The email verification status of the user")
...
async def upload_picture(request:Request)->str:
    ...
    await s3.put_object(cfg.AWS_S3_BUCKET, picture.filename, picture.file.read(), picture.content_type)
    return f"https://s3.amazonaws.com/{cfg.AWS_S3_BUCKET}/{picture.filename}"

@app.post("/api/signup")
async def signup(user:UserSignUp)->str:
    return await auth.signup_endpoint(user.email, user.password, user.email, user.name, user.picture)
